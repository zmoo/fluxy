...
void ClientResult::send() {
	...
	if (error_code != 0) {
		...
		answer << "OK" << std::endl;
		if (type != NONE and data.tellg() != data.tellp()) {
			answer << "DATA: " << ((type == TEXT) ? "TEXT" : "PHP_SERIALIZE") << std::endl << data.str();
		}
	}
	answer << "\r\n";
	client->write(answer);
...
ClientResult::ClientResult(Client *_client) : client(_client), tid(-1), type(TEXT), error_code(0), replicated(false) {
	data.precision(16);
}
